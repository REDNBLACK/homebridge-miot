...
const MiCloudTokenHelper = require('../tools/MiCloudTokenHelper');
    ...
    if (!this.isConnectedToCloud()) {
      if (this._canUseMiCloud()) {
        if (!this.miCloud) {
          ...
          this.miCloud.setServiceToken(MiCloudTokenHelper.getServiceToken());
          //set timeout
        ...
        if (this.miCloud.getServiceToken()) this.logger.info(`Already logged in to MiCloud!`)
        else if (MiCloudTokenHelper.getServiceToken()) {
          this.logger.info(`Re-setting MiCloud service token...`)
          this.miCloud.setServiceToken(MiCloudTokenHelper.getServiceToken())
        }
        else {
          try {
